  ...
  return (
      <div className="min-h-screen bg-page-bg pt-40 pb-20">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="py-20 space-y-0 mb-20">
            {EXPERIENCE.map((job, index) => (
              <motion.div
                ...
                className="grid lg:grid-cols-12 gap-12 py-20 border-b border-white/[0.06] last:border-0"
              >
                <div className="lg:col-span-4 flex flex-col justify-between">
                   <div>
                      <span className="text-xs font-bold text-gray-400 uppercase tracking-widest block mb-4">{job.type}</span>
                      <h2 className="text-4xl md:text-5xl font-plus font-bold text-white leading-tight mb-2">{job.role}</h2>
          ...
          {/* Lessons */}
          <section className="py-40 bg-[#111111] -mx-6 px-6 lg:px-20 text-center">
             <h2 className="text-huge text-white mb-24">Universal Lessons</h2>
             <div className="grid md:grid-cols-3 gap-12 text-left max-w-6xl mx-auto">
               {LESSONS.map((lesson, i) => (
                 <div key={i} className="border-t-2 border-gray-200 pt-8">
                    <span className="text-gray-700 font-plus font-bold text-4xl block mb-6">0{i+1}</span>
                    <p className="text-xl text-white font-bold leading-relaxed">{lesson}</p>
                 </div>
               ))}
             </div>
          </section>
          ...
          >
             <h2 className="text-jumbo text-white mb-12">Focusing on high-stakes challenges.</h2>
